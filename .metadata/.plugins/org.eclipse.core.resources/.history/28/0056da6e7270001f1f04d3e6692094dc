...
public class PurchaseTicketTest extends BaseTest{
	...
	public void purchaseTicketTest(HashMap<String, String> input) throws InterruptedException {
		...
		Assert.assertTrue(schedulepagections.actualOriginMonthText.equalsIgnoreCase(bookpageactions.expectedOriginMonthText));
		Assert.assertEquals(schedulepagections.actualReturnDayText, bookpageactions.expectedReturnDayText);
		Assert.assertTrue(schedulepagections.actualReturnMonthText.equalsIgnoreCase(bookpageactions.expectedReturnMonthText));
		...
		paymentpageactions.payNow(input.get("cardNumber")
				, input.get("cardType")
				, input.get("cardHolder")
				, input.get("expiry")
				, input.get("cvv"));	
	}
